...
The front door for the two-slot workflow: **what runs unattended, and what came
...
grant expiring with the shift. Never dispatch on your own initiative, on a
schedule, or because a batch looks ready.
...
2. Write `PyAutoMind/batches/<YYYY-MM-DD>-<am|pm>.md` from the schema in that
   folder's `AGENTS.md` — members, planned review-minutes, the reason set —
   **before** any session starts. That file is what makes the launch auditable.
...
Then append the outcome to the batch record: `delivered:` (see below) and
**`review-minutes-actual:`**. That second number is the only calibration the
estimate will ever get, so ask for it if the human does not volunteer it.
