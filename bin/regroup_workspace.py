...
def plan(root, state, bundles_root=None, manifest_root=None):
    ...
    for task_root in sorted(task_roots):
        ...
        if 'Generated by PyAutoBrain/bin/worktree.sh' not in before:
            continue  # An unrelated/user-authored activation is not ours to rewrite.
        if not all((task_root / name).is_dir() for name in
                   ('PyAutoBrain', 'PyAutoMind', 'PyAutoHeart', 'PyAutoHands')):
            continue
