...
| `cortex` | The learning function — where the organism finds out what is true: reason over PyAutoCortex (phases, gates, rulings), render its board, and check in on the runs — pull, score against the pre-registered witness, move what came back to awaiting-ruling; never submits a run, never writes a ruling | `bin/pyauto-brain cortex` |
