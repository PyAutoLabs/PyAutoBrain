...
`path`. Science checkouts may live in family folders; organs remain at the workspace
root. CI and task bundles may still place the same checkouts side by side.
...
Make and inspect a journal before changing canonical directory placement:

```bash
python3 PyAutoBrain/bin/regroup_workspace.py plan --root "$PWD" \
  --bundles-root "${PWD}-wt" --state PyAutoMind/tmp/regroup.json
python3 PyAutoBrain/bin/regroup_workspace.py apply --state PyAutoMind/tmp/regroup.json
source activate.sh
python3 PyAutoBrain/bin/regroup_workspace.py verify --state PyAutoMind/tmp/regroup.json
```
...
```bash
python3 PyAutoBrain/bin/regroup_workspace.py rollback --state PyAutoMind/tmp/regroup.json
```
