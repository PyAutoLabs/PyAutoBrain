...
def fixture(tmp_path):
    ...
    (task / 'activate.sh').write_text('# Generated by PyAutoBrain/bin/worktree.sh\nexport PYAUTO_ROOT=' + str(task) + '\n')
    for name in ['PyAutoBrain', 'PyAutoMind', 'PyAutoHeart', 'PyAutoHands']:
        (task / name).mkdir()
...
def test_unrelated_activation_is_not_rewritten(tmp_path):
    root, _, bundles, task = fixture(tmp_path)
    activation = task / 'activate.sh'
    activation.write_text('export USER_PROJECT=keep\n')
    journal = tmp_path / 'journal.json'
    data = migration.plan(root, journal, bundles)
    assert str(activation) not in [row['path'] for row in data['configs']]
