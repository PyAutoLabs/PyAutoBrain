...
> PyAutoCortex (the science body map, the pre-registered tasks, the rulings
...
| `census` | What is the Cortex holding? | task counts by state, the board's section counts, rulings, projects, the last check-in stamp (`--json` for the lot) |
...
| `gates` | What is each gated task waiting on? | every gated task, its refs and the URL each resolves to — read-only and offline |
| `collect [--pull] [--refreshed ISO] [--apply] [--out F] [--task REL]` | What came back, and is it worth reviewing? | one block per task — six scored legs, the readout, a blank ruling line — and exit **1** when any is not HEALTHY. **Default scope: every `submitted \| running` task**; `--task REL` narrows |
...
- `<cortex_root>/scripts/cortex.py` is imported at runtime and is the schema
  API: `load_tasks`, `load_rulings`, `load_projects`, `gates_report`,
  `move_task`. The conductor always reasons with the schema
...
its next task by that task's own summary), then **Projects** (active →
planned → dormant; each card is the folders, the counts and **every open
task of the project, one line each**: the ten-word `Summary:` as the link, a
state pill coloured by `STATE_TONES` — green planned/ready/accepted, yellow
submitted/running/pulled/awaiting-ruling, grey gated, **red** whenever a run
failed, timed out or was voided — and only the facets that state earns) →
**Awaiting ruling** (ordered failures → a ruling is required → clean) →
**Recent rulings**.

The `Running / submitted`, `Ready` and `Gated` sections were dropped on
2026-09-07, with the "N more open phase(s)" fold: they said again, in three
more places, what the Projects block now says once, per project, in colour.
The census still cuts those buckets — they are `collect`'s and the door's
scopes — they are simply not sections of the page. Paths render as accent
chips (`.pathchip`, `--accent` fill with the theme's `--accent-ink`), not as
pink-on-tint inline code.

No `## Where to look` bullets ride on the page — they stay in the task file,
in `census --by-project` and in the check-in printout, which keep the full
per-task tree. The Cortex has no epics of its own (retired 2026-09-04): a
science project *is* the long programme, and `Epic:` on a task header is
only an optional join key to a **Mind** epic (Mind epics really are
sequential; a Cortex task is not — that is why the number went).
...
every gated task, its refs and the URL each ref resolves to. Nothing polls
...
closed, types `python3 scripts/cortex.py move <task> ready`.
...
1. **Sync.** Every project with `status: active` in `projects.yaml`, plus any
   project that owns a task in `submitted | running` (a dormant project with a
   ...
   (`project`, `pulled_at`, `cmd`, `rc`, `tasks_live`) — **merged** into
...
2. **Score + move.** `collect`'s scorer over every live task, then the same
...
5. **Summarise, by project** — printed **last**, so a chat sees it above the
   fold: each project's paths, what its pull did, its task counts, and every
   task a human could act on today with its health verdict and the fenced,
...
root, and every task it would score — and reaches nothing at all. `--project
...
"clean" stops being true the moment the tasks move. That is the whole
...
`--task` it scopes to **every task in `submitted | running`** — the runs the
Cortex believes are out there — and needs no record of any kind. `--task REL`
(repeatable) narrows it to named tasks. Each is scored against **six legs**,
...
| `witness` | a file matching `witness_file` newer than the task's first submission |
...
so a human cannot move tasks on a stale laptop) moves each task
...
Nothing else is written. A task whose run line is still live, or one still
...
RAL · consults an autonomy cap · edits a task except through the Cortex
script's own `move`.
