...
def _mind_home(mind: Path) -> str:
    ...
    if not f.is_file():
        return ""
    # `github:` may follow other keys of the entry (`path:` since the grouped
    # checkouts layout) — scan the whole indented block, not just line one.
    m = re.search(r"^\s{2}PyAutoMind:[ \t]*\n(?:[ \t]{4,}.*\n)*?[ \t]{4,}github:"
                  r"[ \t]*([^\s#]+)",
...
# The third render of the one census (PyAutoBrain#418): the per-organ machine
# surface the cockpit and the phone read (board/_state.py, contract v1). Every
# item's prompt is a payload the page already copies — `/start_dev <path>`,
# `_review_payload`, `REFRESH_PAYLOAD` — composed, never re-derived.
STATE_ITEM_CAP = 20
# In-flight registry statuses that are a human's move. Red: the task cannot
# advance until someone answers; yellow: it is done on its side and waits on a
# merge or a release.
_HUMAN_RED = ("awaiting-input", "blocked")
_HUMAN_YELLOW = ("awaiting-merge", "library-shipped")


def _now_iso() -> str:
    """The render time, whole-second ISO-8601 UTC with a `Z`.

    The census `generated` is date-only (the page stamp), and the contract
    needs a time — so the feed carries the real render time and `--check`
    normalises it out (`_state_body`)."""
...
def _start_here_picks(c: dict) -> list:
    """The Start-here pick lists as the page shows them — high priority, then
    fits-a-slot — de-duplicated, epic members excluded, each list capped at
    PICK_LIST_MAX exactly as rendered."""
    records = sorted(c["records"], key=_pick_key)
    standalone = [r for r in records if not r.get("epic")]
...
def _state_status(c: dict, picks: list) -> str:
    if not (c["records"] or c["in_flight"] or c.get("human_review")
            or c.get("parked") or c.get("planned")):
        return "grey"
    statuses = [(r.get("status") or "").lower() for r in c["in_flight"]]
    if c.get("drift") or any(k in s for s in statuses for k in _HUMAN_RED):
        return "red"
    if (any(k in s for s in statuses for k in _HUMAN_YELLOW)
            or c.get("human_review") or c.get("pending_release") or picks):
        return "yellow"
    return "green"


def _state_items(c: dict, picks: list) -> list:
    home = c.get("home", "")
    blob = f"{home}/blob/main/" if home else ""

    def link(path):
        return blob + path if blob else None

    items = []
    if c.get("drift"):
        items.append({
            "severity": "red",
            "text": f"{len(c['drift'])} prompt(s) need lifecycle reconciliation",
            "url": None, "prompt": REFRESH_PAYLOAD})
    for r in c["in_flight"]:
        status = (r.get("status") or "").lower()
        if any(k in status for k in _HUMAN_RED):
            sev = "red"
        elif any(k in status for k in _HUMAN_YELLOW):
            sev = "yellow"
        else:
            continue
        prs = r.get("prs") or []
    ...
    for r in c.get("human_review") or []:
        ...
        items.append({"severity": "yellow",
                      "text": _one(f"human review: {title}"),
                      "url": link(r["path"]), "prompt": _review_payload(r)})
    for g in c.get("pending_release") or []:
        for pr in g["prs"]:
            items.append({"severity": "yellow",
                          "text": _one(f"pending release: {g['lib']} "
                                       f"{pr['label']} — {pr.get('task', '')}"),
                          "url": pr["url"], "prompt": None})
        for row in g["gated"]:
            items.append({"severity": "yellow",
...
def render_state(c: dict) -> str:
    """`state.json` — the Mind's organ-cockpit feed, as JSON text.

    Status: red when a task waits on a human's answer (awaiting-input /
    blocked) or the prompts have lifecycle drift; yellow when work waits on a
    merge, a release, a review, or there are picks to start; green otherwise;
    grey when the Mind holds nothing at all. The headline is the Start-here
    count line, state-word prefixed when not green."""
    home = c.get("home", "")
    picks = _start_here_picks(c)
    status = _state_status(c, picks)
    line = f"{len(picks)} picks · {c['issued_count']} in flight"
    if status == "grey":
        line = "no tasks held"
    headline = line if status == "green" else f"{status.upper()} — {line}"
    state = build_state(
        organ="mind",
        repo=home.rstrip("/").rsplit("/", 1)[-1] if home else "PyAutoMind",
        status=status,
        headline=headline,
        updated=_now_iso(),
        # the Pages site this feed is served from; relative when underivable
        pages_url=_pages_url(home) or "./",
        items=_state_items(c, picks),
    )
    return json.dumps(state, indent=2) + "\n"


def _state_body(text: str) -> str:
    """The feed minus its `updated` stamp — what `--check` compares.

    `updated` is the render time, so comparing it would make every re-render
    drift (the same reason `_dashboard_body` drops the page stamp)."""
    try:
        obj = json.loads(text)
    except ValueError:
        return text
...
def _page_body(name: str, text: str) -> str:
    return _state_body(text) if name == "state.json" else _dashboard_body(text)
...
def main(argv=None):
    ...
    db = sub.add_parser("dashboard", help="render the census as the Mind task "
                                          ...
                                          "+ the state.json cockpit feed")
    db.add_argument("--check", action="store_true",
                    help="exit 1 if the committed dashboard.md / .html / "
                         "state.json have drifted from what this run renders "
                         "(the generation date and the feed's `updated` are "
                         "ignored, so a re-render on an unchanged Mind is not "
                         "drift)")
    ...
    if a.cmd == "dashboard":
        ...
        # The third is the organ-cockpit feed (state.json, board/_state.py).
        ...
        if a.check:
            ...
            for name, want in pages.items():
                ...
                if _page_body(name, on_disk) != _page_body(name, want):
                    stale.append(name)
            if not stale:
        ...
        if a.as_json:
            ...
            print(json.dumps({"census": summary, "page": pages["dashboard.md"],
                              ...
                              "state": json.loads(pages["state.json"]),
